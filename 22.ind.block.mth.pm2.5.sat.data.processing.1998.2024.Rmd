...
compliance_summary <- final_all_years_cbind_df_sh_mapped_long %>%
  ...
  summarise(
    ...
    us_epa_compliant = ifelse(first(year) >= 2021, sum(avg_pm2.5 <= 5, na.rm = TRUE), sum(avg_pm2.5 <= 10, na.rm = TRUE)),
...
p2 <- ggplot(
  ...
  labs(
    title = "Compliance Stripes: All Indian States Across 27 Years (1998 to 2024)",
    subtitle = "WHO Guideline (≤5 µg/m³ from 2021), before 2021: ≤10 µg/m³",
    x = "Year",
    y = NULL,
...
# Save with appropriate dimensions for 27 states
ggsave("[test4]compliance_stripes_chart_all_states_who.png", p2, 
       width = 54, height = 34, dpi = 520, bg = "white", limits = FALSE)
