...
compliance_summary_non_state <- final_all_years_cbind_df_sh_mapped_long %>%
  group_by(year, month) %>%
  summarise(
    national_compliant = sum(avg_pm2.5 <= 40, na.rm = TRUE),
    who_compliant = sum(avg_pm2.5 <= 5, na.rm = TRUE), 
    us_epa_compliant = sum(avg_pm2.5 <= 9, na.rm = TRUE),
  ...
  mutate(
    ...
    us_epa_pct = (us_epa_compliant / total_subdistricts) * 100,
    ...
    us_epa_text_color = ifelse(us_epa_pct < 60, "black", "white")
  )

# National guideline heatmap
p1 <- ggplot(compliance_summary_non_state, 
             aes(x = factor(year), y = month_name, fill = national_pct)) +
  geom_tile(color = "white", linewidth = 2) +
  geom_text(aes(label = sprintf("%.2f%%", national_pct), color = national_text_color),
...
p2 <- ggplot(compliance_summary_non_state, 
             aes(x = factor(year), y = month_name, fill = who_pct)) +
  geom_tile(color = "white", linewidth = 2) +
  geom_text(aes(label = sprintf("%.2f%%", who_pct), color = who_text_color), 
            size = 6, fontface = "bold") +
  scale_fill_gradient2(
    low = "#d73027", 
    mid = "#fee090", 
...
# WHO guideline heatmap
p3 <- ggplot(compliance_summary_non_state, 
             aes(x = factor(year), y = month_name, fill = us_epa_pct)) +
  geom_tile(color = "white", linewidth = 2) +
  geom_text(aes(label = sprintf("%.2f%%", us_epa_pct), color = us_epa_text_color), 
            size = 6, fontface = "bold") +
  scale_fill_gradient2(
    low = "#d73027", 
    mid = "#fee090", 
  ...
  scale_color_identity() +  # Use the actual colors specified
  labs(
    title = "US EPA National Annual Average PM2.5 Standard (≤9 µg/m³)",
    x = "Year",
    y = NULL
  ) +
  theme_minimal(base_size = 22) +
...
combined_plot <- p1 / p3 + 
  plot_annotation(
    ...
    subtitle = "Percentage of 5,968 subdistricts meeting national and US EPA PM2.5 standards (1998-2024) Months show clear seasonal patterns, with monsoon months (June-September) exhibiting better compliance.There is also a diminishing\n 'greeness' in the national plot as we move from 1998 towards 2024. In any given month, later years have gotten 'on average' worse (especially October to April months) in terms of compliance compared to previous years.\nWhile the US EPA guideline comparison is clear - extremely low levels of compliance under this stricter standard and that too only few non-winter months in early years. From US Standards, India's picture looks bleak.",
    caption = "Data: Population-weighted monthly PM2.5 concentrations across Indian subdistricts\nData Source: https://github.com/AarshBatra/biteSizedAQ/tree/main/22.ind.block.mth.pm2.5.sat.data.processing.1998.2024\nIndia National Annual Average PM2.5 Standard: 40 µg/m³ annual average | US EPA National Annual Average PM2.5 Standard: 9 µg/m³\nGreen = high compliance, Red = low compliance | Each cell shows percentage of Indian Subdistricts meeting the respective threshold\nGraphic: Aarsh Batra (github.com/AarshBatra/biteSizedAQ)",
    theme = theme(
      ...
      plot.caption = element_text(size = 17.5, hjust = 0, color = "grey40", 
                                  lineheight = 1.4, margin = margin(t = 60), face = 
                                    "italic")
    )
  )
...
ggsave("./plots_for_jeff_smith/pm25_compliance_heatmap_ind_nat_us_epa_2_limits.png", 
       combined_plot, 
       width = 30,
       height = 22,
       dpi = 300,
...
#### IGP compliance stripes chart (YEAR-MONTH-STATE wise)=======================
library(ggplot2)
library(dplyr)
...
# compliance summary by year - month - state
...
p1 <- ggplot(
  ...
  labs(
    ...
    x = "Year",
    y = NULL,
    caption = "Data: Population-weighted monthly PM2.5 concentrations across Indian subdistricts\nData Source: https://github.com/AarshBatra/biteSizedAQ/tree/main/22.ind.block.mth.pm2.5.sat.data.processing.1998.2024\nIndia National Annual Average PM2.5 Standard: 40 µg/m³\nGreen = high compliance, Red = low compliance | Each cell shows percentage of Indian Subdistricts meeting the National PM2.5 Standard in a given state in a given year and month\nGraphic: Aarsh Batra (github.com/AarshBatra/biteSizedAQ)"
  ) +
  ...
  theme(
    ...
    plot.caption = element_text(hjust = 0, size = 26, color = "grey40", margin = margin(t = 40), lineheight = 1.2, face = "italic"),
    panel.grid = element_blank(),
...
ggsave("./plots_for_jeff_smith/compliance_stripes_state_wise_wrt_ind_nat_stan.png", p1, 
...
p2 <- ggplot(
  ...
  labs(
    title = "Compliance Stripes: All Indian States Across 27 Years (1998 to 2024)",
    subtitle = "WHO Guideline (≤5 µg/m³)",
    x = "Year",
    y = NULL,
    caption = "Data: Population-weighted monthly PM2.5 concentrations across Indian subdistricts\nData Source: https://github.com/AarshBatra/biteSizedAQ/tree/main/22.ind.block.mth.pm2.5.sat.data.processing.1998.2024\nWHO Annual Average PM2.5 Guideline: 5 µg/m³\nGreen = high compliance, Red = low compliance | Each cell shows percentage of Indian Subdistricts meeting the WHO PM2.5 Guideline in a given state in a given year and month\nGraphic: Aarsh Batra (github.com/AarshBatra/biteSizedAQ)"
  ) +
  theme_minimal(base_size = 16) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 52, margin = margin(b = 8)),
...
# Save with appropriate dimensions for 27 states
ggsave("./plots_for_jeff_smith/compliance_stripes_state_wise_wrt_who_guideline.png", p2, 
       width = 54, height = 34, dpi = 520, bg = "white", limits = FALSE)
...
p3 <- ggplot(
  compliance_summary_binned,
  aes(x = factor(year), y = month_name, fill = compliance_bin_us_epa)
) +
  geom_tile(color = "white", linewidth = 0.3, width = 0.95, height = 0.95) +
  scale_fill_manual(
    values = stripe_colors,
    name = "% of Subdistricts\nMeeting US EPA Standard",
    drop = FALSE
  ) +
  scale_x_discrete(
    breaks = seq(1998, 2024, 2),  # Show every 2 years
  ...
  labs(
    title = "Compliance Stripes: All Indian States Across 27 Years (1998 to 2024)",
    subtitle = "US EPA Annual Average PM2.5 Standard (≤9 µg/m³)",
    x = "Year",
    y = NULL,
    caption = "Data: Population-weighted monthly PM2.5 concentrations across Indian subdistricts\nData Source: https://github.com/AarshBatra/biteSizedAQ/tree/main/22.ind.block.mth.pm2.5.sat.data.processing.1998.2024\nUS EPA National Annual Average PM2.5 Guideline: 9 µg/m³\nGreen = high compliance, Red = low compliance | Each cell shows percentage of Indian Subdistricts meeting the US EPA National PM2.5 Guideline in a given state in a given year and month\nGraphic: Aarsh Batra (github.com/AarshBatra/biteSizedAQ)"
  ) +
  theme_minimal(base_size = 16) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 52, margin = margin(b = 8)),
    plot.subtitle = element_text(hjust = 0.5, size = 33, color = "grey30", margin = margin(b = 20)),
    plot.caption = element_text(hjust = 0, size = 26, color = "grey40", margin = margin(t = 40), lineheight = 1.2),
    panel.grid = element_blank(),
    panel.spacing.x = unit(1.5, "lines"),
    panel.spacing.y = unit(1.5, "lines"),
    axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, size = 22),
...
# Save with appropriate dimensions for 27 states
ggsave("./plots_for_jeff_smith/compliance_stripes_state_wise_wrt_us_epa_guideline.png", p3, 
       width = 54, height = 34, dpi = 520, bg = "white", limits = FALSE)
