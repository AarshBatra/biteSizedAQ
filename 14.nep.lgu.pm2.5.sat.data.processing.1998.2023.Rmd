...
nep_gadm3 <- st_read(str_c(here(), "/data/raw/other.datasets/shpfiles/nepal_gadm3/nep_gadm3.gpkg"))
nep_gadm3 <- nep_gadm3 %>%
...
ref_admin_level_shp_file <- nep_gadm3

#> [To be set by user] set the ref admin layer
ref_shp_file_to_be_rasterized <- nep_gadm3
...
plt_pol_2023 <- final_all_years_cbind_df_sh_mapped  %>%
  ...
  labs(fill = expression("Annual Average"~PM[2.5]~"(micrograms per cubic metre)"),
       title = expression("Block level"~PM[2.5]~"pollution in Nepal, 2023")) +
...
## For analysis coalesce certain lgu alternative names with the lgu name col
