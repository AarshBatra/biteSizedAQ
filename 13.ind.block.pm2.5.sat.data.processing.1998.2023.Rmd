...
final_all_years_cbind_df_sh_mapped %>%
  readr::write_csv(paste0(here(), "./data/processed/13.ind.block.pm2.5.sat.data.processing.1998.2023/", "ind_block_level_pop_weighted_pol_1998_2023.csv"))
...
plt_pol_2023 <- final_all_years_cbind_df_sh_mapped  %>%
  ...
  st_as_sf() %>% ggplot() + geom_sf(mapping = aes(fill = avg_pm2.5_2023), color = "transparent") + scale_fill_viridis(option = "magma", direction = -1, breaks = c(0, 5, 10, 20, 40, 60, 80, 120), limits = c(0, 120)) +
  ...
  labs(fill = expression("Annual Average"~PM[2.5]~"(micrograms per cubic metre)"),
       title = expression("Block level"~PM[2.5]~"pollution in India, 2023")) +
