...
#### create a gif out of first and last year
...
# Read the two images
img1 <- image_read("./15.ind.block.mth.pm2.5.sat.data.processing.1998.2023/monthly.india.pm2.5.snapshot.all.yrs/mth_avg_pm2.5_india_snapshot_1998.png")
img2 <- image_read("./15.ind.block.mth.pm2.5.sat.data.processing.1998.2023/monthly.india.pm2.5.snapshot.all.yrs/mth_avg_pm2.5_india_snapshot_2023.png")
...
image_write_video(image_join(img_list), "output.mp4", framerate = 1)



```

## [TESTING SPACE, CAN BE IGNORED COMPLETELY]

```{r}

# # reorder col names so that each year's 12 month show in chronological order, then same for next year and so on till last year.
# # 1. Extract the column names
# cols <- colnames(final_all_years_cbind_df_sh_mapped)
# 
# # 2. Identify the 'avg_pm2.5_' columns
# pm_cols <- cols[str_detect(cols, "^avg_pm2\\.5_")]
# 
# # 3. Create a tibble to help with ordering
# pm_cols_ordered <- tibble(
#   colname = pm_cols,
#   year = str_extract(pm_cols, "\\d{4}") %>% as.integer(),
#   month = str_extract(pm_cols, "(?<=_\\d{4}_).*")
# ) %>%
#   mutate(
#     month = factor(month, 
#                    levels = c("january", "february", "march", "april", "may", "june",
...
#   select(state_name, district_name, subdistrict_name, subdistrict_population, all_of(pm_cols_ordered))
